...
L'équivalent des tests effectués avec les bibliothèques dynamiques du système d'exploitation
ont été effectué avec les bibliothèques JavaScript pour montrer la coexistence de bibliothèque
...
Les bibliothèques au sein d'un application sont chargé de façon unique.
...
\texttt{D} exporte une variables globale. Les bibliothèques \texttt{B} et \texttt{C} dépendent
de \texttt{D}. Le programme principale utilise les bibliothèques \texttt{B} et \texttt{C}.
La figure~\ref{fig:libdep_experience_global_var} est une représentation graphique des dépendences
présente dans l'expérience.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.75]{figures/libdep_experience_global_var}
  \caption{Cette figure présente le graphe de dépendence des bibliothèques \texttt{B}, \texttt{C} et \texttt{D}
    avec l'application \texttt{Programme}.}
  \label{fig:libdep_experience_global_var}
\end{figure}

La notation \texttt{X.Y.val} exprime une références à la variables \texttt{val}
de \texttt{Y} à partir de la bibliothèque \texttt{X}.

Les étapes exécutés par le programme dans l'expériences sont les suivantes:
\begin{enumerate}
...
Les valeur de \texttt{B.D.value} et \texttt{C.D.value} doivent être égales à l'étape 2.
Les variables sont partagé si la valeur \verb+B.D.value == C.D.value+ après l'étape
3 de l'expérience.\\
\begin{figure}[ht]
  \centering
  \lstset{frame=single}
...
\end{mplisting}
  \caption{Les résultat de l'expérience avec des bibliothèques natives du système d'exploitation sous Linux.}
...
que chacune des variables globales d'une bibliothèques partagées existent qu'en une seule
instance.
...
\section{Conclusion}
Ce chapitre a traité du fonctionnement de l'éditeur de lien et des bibliothèques dynamiques.
