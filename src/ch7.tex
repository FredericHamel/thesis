...
d'évaluation recycle trois programmes de test (benchmarks). Nous avons
...
Le but est de prouver que la diffusion de code compilé est plus rapide que
la diffusion de code interprété. L'avantage de transmettre le code compilé
comparé aux codes interprétés. Nous avons aussi testé la diffusion des modules
...
respectivement dans les ordres de 0.1 seconde, 1 seconde, 10 secondes. La
figure~\ref{fig:gambit-arctic-interp} montre cette tendance dans les temps
d'exécutions. Le but de l'arrangement est de montrer un lien entre la taille du
...
diffusion de code est indépendante de la plateforme et de l'architecture de la
machine.
...
Les temps dans les tests sont mesurés en millisecondes (ms).
Les éléments qui affecte les résultats sont le réseau, la vitesse de
la machine destination. Le réseau a affecté 1/10 des transmissions
effectuées dans l'ensemble des tests dans l'ordre des millisecondes.
Cela est observé par une variance élevée dans le temps de transfert.
Pour la même taille de transmission, le temps de transfert est plus
grand pour plusieurs exécutions.
...
\end{tabular}
  \caption{Ce test est identique à \ref{fig:tictoc-arctic}, mais avec un réseau de $10Mbit/s$.}
\end{figure}


\subsection{Dépendance entre la taille du module et les temps}
Le temps d'installation du module sur le nœud destination
...
Le temps d'exécution ne semble pas suivre la même tendance. Dans les
cas compilés, l'expérience 40K est plus rapide que 4K dans l'exécution.
Cela s'explique par le fait que la boucle qui paramètre le nombre de
fois que la procédure est répétée est interprétée. Le nombre d'appels
à la procédure du test de performance de 40K requis pour que le temps d'exécution
interprété soit dans l'ordre d'une seconde est 1 qui est moins que
le test 4K. Il y a moins de code interprété exécuté dans le test 40K.
Le test 4K prend moins de temps que le test 400K.
...
Le temps d'exécution total (RPC) des programmes de tests exploitant la
diffusion de code compilé, nécessitant l'installation et la compilation du
module, est plus lent que la version interprétée. Le facteur qui prend le plus
de temps dans le contexte compilé est l'installation et la compilation des
modules diffusés. L'exécution seule est plus rapide que dans le cas interprété
par un facteur d'environ 10.

Dans le cas où les modules ont déjà été installés et compilés le temps global
d'exécution (RPC) des programmes de tests est plus rapide dans compilé.
...
transmissions et est lié à des facteurs du réseau.
...
c'est-à dire après que les modules ont été installés et compilés.
...
% Execution & $      10.5(\sigma =  .0)$ & $        .2(\sigma =  .0)$ & $    1468.5(\sigma =  .2)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~152.5(\sigma = 0.0)$ & $~~~~~971.0(\sigma = 0.0)$ & $~~~12268.7(\sigma = 0.0)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~~11.6(\sigma = 0.0)$ & $~~~~~~~0.1(\sigma = 0.0)$ & $~~~~1453.1(\sigma = 0.0)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~~10.5(\sigma = 5.4)$ & $~~~~~~~0.2(\sigma = 0.1)$ & $~~~~1468.5(\sigma = 13.8)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~~11.6(\sigma = 3.0)$ & $~~~~~~~0.1(\sigma = 0.1)$ & $~~~~1453.1(\sigma = 4.7)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~~10.2(\sigma = 5.1)$ & $~~~~~~~0.2(\sigma = 0.1)$ & $~~~~1471.0(\sigma = 12.4)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~~19.2(\sigma = 1.8)$ & $~~~~~~~0.2(\sigma = 0.0)$ & $~~~~1478.9(\sigma = 2.7)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~~~9.8(\sigma = 4.7)$ & $~~~~~~~0.2(\sigma = 0.1)$ & $~~~~1466.9(\sigma = 13.3)$\\\hline
% \end{tabular}
...
% Execution & $~~~~~152.5(\sigma = 3.5)$ & $~~~~~971.0(\sigma = 1.4)$ & $~~~12268.7(\sigma =~6.0)$\\\hline
% \end{tabular}
% \subsection{dummy 2}
...
% Execution & $~~~~~~11.7(\sigma = ~~3.0)$ & $~~~~~~~0.1(\sigma = ~~0.0)$ & $~~~~1452.9(\sigma = ~~4.0)$\\\hline
% \end{tabular}
%   \caption{Ce sont les temps d'exécution et de transmission de module de différente taille entre Gambit et Arctic
%   dans le cas ou les modules sont présents sur chaque nœud.}
%   \label{fig:gambit-arctic}
...
% Transmission & $~~~~~~27.8(\sigma = ~~3.2)$ & $~~~~~~47.3(\sigma = ~~2.8)$ & $~~~~~~~2.2(\sigma = ~~0.7)$\\\hline
% Execution & $~~~~~~19.3(\sigma = ~~1.5)$ & $~~~~~~~0.2(\sigma = ~~0.0)$ & $~~~~1478.9(\sigma = ~~2.6)$\\\hline
% \end{tabular}
%   \caption{Cette expérience est le même que \ref{fig:gambit-arctic}, sauf que le transfert de module est fait entre
%   un système ARM (tictoc) et x86 (arctic).}
% \end{figure}
...
% Execution & $      10.9(\sigma = 0.0)$ & $       0.2(\sigma = 0.0)$ & $    1468.3(\sigma = 0.2)$\\\hline
% \end{tabular}
...
% Execution & .010925736427307129 & 3.169668470971172e-5\\\hline
% \end{tabular}\\[2ex]
% \begin{tabular}{|l|c|c|}
% \hline\\&Moyenne&Variance\\\hline
...
% Execution & 1.8299579620361327e-4 & 3.705560946841479e-9\\\hline
% \end{tabular}\\[2ex]
% \begin{tabular}{|l|c|c|}
% \hline\\&Moyenne&Variance\\\hline
...
% Execution & 1.4682691979408264 & 1.9840058814737067e-4\\\hline
% \end{tabular}\\[2ex]
% \end{center}
% \begin{center}
...
% Execution & .01144924258241559 & 9.258678621880817e-6\\\hline
% \end{tabular}\\[2ex]
% \begin{tabular}{|l|c|c|}
% \hline\\&Moyenne&Variance\\\hline
...
% Execution & 1.436601770986425e-4 & 2.0670921404047487e-9\\\hline
% \end{tabular}\\[2ex]
% \begin{tabular}{|l|c|c|}
% \hline\\&Moyenne&Variance\\\hline
...
% Execution & 1.4543538329624894 & 3.541463525408517e-5\\\hline
% \end{tabular}\\[2ex]
