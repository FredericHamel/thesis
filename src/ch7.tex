...
\begin{itemize}
  ...
  \item \MMM[ARM/Linux] est un Raspberry Pi \texttt{ARM} roulant Linux.
\end{itemize}
...
La machine \MMM[ARM/Linux] est la machine la moins performante. Elle est d'une
...
exécuté 20 fois, les valeurs extrêmes (la plus grande est la plus petite) sont
...
% Étant donné que le réseau a perturbé 10\% des transmissions effectuées dans
% l'ensemble des tests dans l'ordre des millisecondes, nous avons enlevé 5\%
% des valeurs minimales et 5\% des valeurs maximales pour minimiser l'effet
% aléatoire du réseau.  Cela est observé par une variance élevée dans le temps
% de transfert.  Pour la même taille de transmission, le temps de transfert est
% plus grand pour plusieurs exécutions.
...
grands sont généralement pour \MMM[ARM/Linux]. Cela peut être expliqué par le
...
\MMM[x86/macOS] et de 25-30ms pour \MMM[ARM/Linux] (un processeur et une
interface réseau moins performants). Les temps mesurés sur le nœud destination
sont essentiellement les mêmes.

Dans le scénario INTERPRETED le message contient une représentation du code à
...
\MMM[ARM/Linux] (encore une fois le temps est affecté par la performance du
...
\caption{Les temps dans le scénario INTERPRETED avec
  \MMM[x86/macOS] ou \MMM[ARM/Linux] comme le nœud de départ}
...
% même système \MMM[x86/Linux] pour la stabilité d'exécution.  Les temps de
% transmission croît avec croissent avec la taille du programme.
...
\subsection{Comparaison entre les scénarios INTERPRETED et STEADY-STATE}
...
STEADY-STATE est 22x plus plus petit pour \MMM[x86/macOS] et \MMM[ARM/Linux]
...
indique que le temps total RPC pour \MMM[x86/macOS] et \MMM[ARM/Linux] est
...
Dans le cas où les modules ont déjà été installés et compilés, l'exécution
