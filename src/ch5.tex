...
contient plusieurs procédures pour tester le bon fonctionnement d'un module.
Les tests unitaires pour un module nommé \lstcode{A} sons dans le sous-module
\lstcode{A/test}.
...
La plupart des systèmes de module ne distinguer pas les versions d'un même
module et ne permette l'installation que d'une seule version. Les systèmes de
module permettent la gestion de différent préfixe dans lequel les modules sont
...
\textit{mancoosi}, un projet Européen de recherche dans le 7e cadre de
recherche (FP7) de la Commission européenne, pour gérer les contraintes de
...
%
L'organisation des bibliothèques dans Go est plus dans un contexte
environnement dont la racine est spécifier par la variable d'environnement
...
dépendances lors de l'installation de nouveaux modules.
