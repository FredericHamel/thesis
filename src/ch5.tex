...
Un \lstcode{<module-ref>} est l'identifiant unique qui est utilisé pour référer
...
\lstcode{<module-ref>} est séparé en trois parties: le \lstcode{hostname}, le
\lstcode{path} et l'étiquette qui donne la version. Dans le cas d'un module
hébergé le \lstcode{hostname} contient le nom de domaine du site. Dans le cas
local, ce champs est vide.  La grammaire\ref{lst:module-ref->grammar} décrit la
syntaxe formelle du \lstcode{<module-ref>} dans le cas hébergé et local.
...
\lstcode{\~\~instlib}. Par défaut c'est le même que le répertoire des
...
L'installation des module peut être soit à partir d'un serveur de révision
