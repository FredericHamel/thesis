...
possible que l'espace de nom déclaré dans \lstcode{angle2/angle2#.scm} ne
corresponde pas à celui utilisé dans \lstcode{angle2/angle2.scm}.
...
L'expansion de la forme \lstcode{import} dépend du type de module qui
...
dépendent s'il est dans un \lstcode{define-library} ou dans un programme principal. Dans
...
\end{mplisting}
  \caption{Différent \lstcode{\#\#namespace} générés par
...
correspond à l'importation de \lstcode{(A C)}. Cela permet au sous
...
(define-library (A B)
  ...
  (import (..))) ;; => (import (A))
\end{mplisting}
  \caption{Importation relative du module \lstcode{(A C)}}
