...
du module. Les différents méta infos sont \lstcode{##cc-options},
\lstcode{##ld-options}, \lstcode{##ld-options-prelude}, \lstcode{##pkg-config}
et \lstcode{##pkg-config-path}. Ces méta infos ne sont utile que pour les modules
compilés.

\begin{itemize}
  \item Les \lstcode{##cc-options} sont ajouter au options de la command qui
    invoque le compilateur C.

  \item Les méta infos \lstcode{##ld-options} et \lstcode{#3ld-options-prelude}
    ajoute les paramètres à la command qui invoque l'éditeur de lien.
    Les paramètres dans \lstcode{#3ld-options-prelude} précèdent ceux
...
\lstcode{define-library} est la possibilité d'avoir un \lstcode{import} relatif
