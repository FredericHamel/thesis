...
avec un système de type dynamique, cela signifie que les variables peuvent
contenir n'importe quels types. Il supporte plusieurs paradigmes de
programmation comme fonctionnelle, impérative et méta. Chaque expression est
représentée sous forme préfixée et fortement parenthésé.  Un programme Scheme est
une séquence de liste parenthésé en forme préfixée, ce sont des
\textit{s-expressions}.  Chaque \textit{s-expression} correspond soit à une
constante, une application de procédure ou une application de macro. Les macros
sont liées au paradigme de méta programmation qui manipule les expressions.
...
précède les opérandes. Cette forme est plus utilisée dans les langages de la famille
LISP.
...
Les différents types primitifs que les valeurs peuvent prendre
en Scheme sont \lstcode{boolean}, \lstcode{pair}, \lstcode{symbol},
...
Les procédures sont des objets de première classe, cela signifie qu'elles
peuvent être manipulées comme n'importe quels types de données. Elles peuvent
être passées en tant que paramètre à une procédure et retournées en tant que
résultat.  Certaines fonctions -- telles que \lstcode{for-each}, \lstcode{map} et
\lstcode{fold} -- bénéficient que les procédures sont des objets de première classe.

Les procédures et les fonctions sont définies par le constructeur
...
La programmation méta est présente dans plusieurs langages comme C, C++, LISP,
...
macro avec \lstcode{define-macro}.  Pour les implémentations de Scheme ne
