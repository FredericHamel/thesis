...
La \textit{modularisation} du code mobile est un concept intéressant dans les
systèmes distribués.  L'approche employée, pour implémenter une application
avec ce concept, utilise le système Termite, une implémentation du protocole
d'Erlang avec le langage Scheme. Erlang et Termite sont des langages de
programmation fonctionnelle pour concevoir les systèmes distribués. La syntaxe
de Scheme est simple et a la propriété d'homoiconicité, c'est-à-dire que la
représentation des programmes est aussi une structure de donnée.  La pièce qui
manque dans Termite est la modularisation des applications. Ce chapitre
introduit la syntaxe de Scheme et des modules de chacune de ses révisions
(R4RS, R5RS, R6RS et R7RS). Le choix de la syntaxe pour les modules est
important.
...
syntaxe préfixe parenthétisée de Lisp.  Un programme Scheme est une séquence de
...
\lstcode{fold}, en tire profit.

Les procédures sont définies par la forme \lstcode{lambda} qui prend une liste
...
La méta programmation est présente dans plusieurs langages comme C, C++, LISP,
...
\lstcode{defmacro} de LISP. Elle accepte en entrée des \textit{s-expression}s et
...
macro avec \lstcode{define-macro}.  Pour les systèmes Scheme n'offrant
pas la forme \lstcode{define-macro}, il assez simple d'implémenter
...
\lstcode{define-syntax} ne sont pas pertinentes à ce mémoire.
...
une expansion des macros. Le \lstcode{include} d'un fichier n'effectue qu'une
analyse lexicale qui est effectuée par la procédure \lstcode{read}. L'évaluation
...
et à l'évaluation. La procédure \lstcode{load} donne accès aux procédures définit
...
\begin{itemize}
  ...
  \item La déclaration \lstcode{cond-expand} est une extension du \texttt{SRFI-0}
...
syntaxiquement en partant de R4RS à R7RS. Les module R6RS ont été mentionnés,
car ils font parti de l'évolution de Scheme. Ils ne sont pas pertinent pour
dans l'implémentation du système de module.  La forme \lstcode{define-library}
