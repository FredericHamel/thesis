...
qui permet de modifier le flot de contrôle du programme comme le \lstcode{if}.
Le \lstcode{while} et \lstcode{for} sont utilisés pour les boucles. Ces
...
modifiés. Scheme a la forme \lstcode{begin} qui permet l'exécution
séquentielle des expressions et la forme \lstcode{set!} pour réassigner une
variable.
...
fonctionnel, le \lstcode{if} est une expression qui retourne le résultat de la
...
La programmation méta est un paradigme lié à l'utilisation de macro qui sont
...
\section{Structure des Bibliothèque}
...
le module courant par la procédure \lstcode{load}. Certaines
implémentations de Scheme ont la forme spéciale \lstcode{include} qui peut s'ajouter
...
\begin{itemize}
  \item Ce modèle de chargement n'est pas à l'abri des chargements multiples
    d'un module qui mène soit de la duplication de code (dans le cas de \lstcode{include})
    ou la réévaluation d'un code (dans le cas de \lstcode{load}).
...
Dans un module, il y a du code qui est exécuté à l'expansion (les macros) et à
l'évaluation. La procédure \lstcode{load} donne accès au
...
contenu du fichier \lstcode{foo#.scm}. L'exemple \ref{fig:r4rs_fact}
montre un exemple de module simple n'utilisant que \lstcode{load}.
...
module. Le nom du module peut contenir un numéro de version. La sous forme
\lstcode{export} spécifie la liste des associations définit ou importé dans le
module. La sous forme \lstcode{import} spécifie une liste de dépendances dans
laquelle chaque dépendance spécifie:
...
d'autre module. Les déclarations d'un module ne peuvent pas être en conflit avec
...
C'est ce qui lie les déclarations au module.


%Cette syntaxe 
%est fortement lié à ce module. Chaque module a son propre
%espace de environnement. sdans l'espace de nom du module sert d'espace de nom pour les associations
