...
positifs. Une liste vide \lstcode{()} est l'équivalent de ne pas spécifier
la version. La seconde partie du module est la spécification de la liste
des associations exportées par la sous forme \lstcode{export}. Chaque élément
de cette liste est soit un identifiant ou un une sous forme \lstcode{rename}
qui renomme l'identifiant exporté. La troisième partie, le \lstcode{import},
donne la liste des dépendances du module. Chaque dépendance spécifie:
...
empêche le chargement multiple d'un module.  La syntaxe du \lstcode{import} et
\lstcode{import} permet de manipuler les noms des symboles importés et exportés.
...
Chaque spécification d'import permet un niveau d'importation qui peut être soit:
\begin{center}
  \begin{mplisting}{0.9}
run
expand
...
\end{mplisting}
\end{center}
où \lstcode{<level>} est un entier exacte. Un niveau d'importation 0
correspond à \lstcode{run} et un niveau d'importation de 1 correspond
\lstcode{expand}. La syntaxe pour l'importation est décrit dans la
spécification R6RS\cite{Scheme:R6RS}. Cela permet un grand contrôle sur le mode
d'importation au prix d'une sémantique plus complexe.
